...
<body style="font-family: 'Ubuntu', sans-serif;height:100%;padding: 0;margin: 0;font-size: 22px;">
    ...
    <div class="container" style="background-image: url('images/phon3.jpg');justify-content: center;display: flex;" id="cont">
        <form action="login.php" method="post" enctype="multipart/form-data" class="form">
            ...
            <br>
            <label for="fileToUpload" class="custom-file-upload" style="margin-top: 5px"> <span class="lnr lnr-cloud-upload">  </span>Upload</label>
            <input type="file" name="fileToUpload" id="fileToUpload"> <!--special file css design-->
            <br>
                Write the topic of your video:
            <br>
            <input type="text" name="videoname" maxlength="100"> <!--max length is set-->
            <br>
            Select the grade:
            <br>
            <div class="box">
                <select id="grades" name="grades" style="vertical-align: top;" onchange="myFunction()"> <!--function if value is changed-->
                    <option value="" selected disabled hidden>Choose a grade</option>
                    <?php $query=mysqli_query($db, "SELECT DISTINCT grade FROM grades");
                    while ($row = mysqli_fetch_array($query)) { //gets each grade number without repetition
                        ?><option value="<?php echo $row['grade'] ?>"><?php echo $row['grade']; ?></option><?php }?> 
                </select>
            </div>
            <div class="box">
                <select id="groups" name="groups[]" style="display: none;vertical-align: top;margin-top:5px;overflow-y: auto;" 
                multiple size='2'> <!-- 2 groups only -->
            ...
            <div class="box">
                <select id="advanc" name="adv[]" style="display: none;vertical-align: top;margin-top:5px" 
                multiple size="4"> <!-- 4 groups -->
                    <?php $query=mysqli_query($db, "SELECT DISTINCT studygroup FROM grades WHERE grade=11 or grade=12");
                    while ($row = mysqli_fetch_array($query)) { 
            ...
            <script type="text/javascript">
                function myFunction() {
                    ...
                    if (x == "11" || x=="12") {
                        document.getElementById("advanc").style.display = "block";//next select shows up
                        document.getElementById("groups").style.display = "none";
                    } else{
                        document.getElementById("groups").style.display = "block";//next select shows up
            ...
            <input style="vertical-align: top;" class="button2" type="submit" value="Upload Video" name="uploadvideo">
